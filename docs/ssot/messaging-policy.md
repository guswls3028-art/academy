...
13-A. **접수와 최종 전달 분리** — `NotificationLog.status=sent`와 provider ID는 공급사 접수 근거다. 권한 있는 교직원이 상세에서 명시적으로 최종 상태를 확인할 때만 저장된 group ID로 Solapi 메시지 목록을 read-only 조회한다. provider `2000`은 접수, `3000`은 통신사 결과 대기, 모든 메시지의 `4000`만 최종 전달 성공이며 다른 terminal 코드는 실패다. 조회 오류는 접수 상태를 전달 성공으로 승격하지 않는다. provider 호출 경계 뒤 응답을 받지 못해 ID가 없는 `ambiguous`는 저장된 원문 오류의 종류와 관계없이 사용자 projection을 `provider_unconfirmed`로 유지한다. 조회 식별자가 실제로 없으므로 최종 상태는 추측하지 않고 확인 불가로 표시하며, 자동 재발송·환불·상태 강제변경 없이 operations의 전체 미확정 수에 남긴다.
13-B. **수동 요청 exact trace** — 한 번의 사용자 발송 동작은 UUID `client_request_id` 하나를 학생·학부모 요청에 공통 사용하고 서버는 API 접수마다 별도 UUID `batch_id`를 발급한다. outbox/SQS/worker log는 `request_id`, `batch_id`, `origin_type=manual_send`, `origin_id=request_id`, 요청 직원 ID를 보존한다. worker는 trace version이 있는 payload의 UUID·origin·`occurrence_key=request:<request_id>:batch:<batch_id>`를 다시 exact 검증하며 drift는 provider 호출 전에 terminal 실패한다. 유효 번호 0과 durable 큐·예약 접수 0은 2xx 성공으로 반환하지 않는다. `request_id` 로그 필터는 현재 tenant의 manual origin만 exact 조회해 참관 사본 등 다른 provenance를 섞지 않는다.
...
20. **개인정보 없는 incident trace** — outbox와 worker log는 원문 번호 대신 `MESSAGING_TENANT_BINDING_KEY` HMAC `recipient_fingerprint`를 저장하고, `origin_type`/`origin_id`와 별도 `batch_id`로 Excel job·수동 request/batch·domain object를 연결한다. terminal payload에는 이 비식별 메타데이터와 기존 dispatch/business key만 남긴다. 키 순환 중 조회는 fallback key 지문도 함께 계산한다.
