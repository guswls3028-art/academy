...
`work_record_date_anomalies` 룰은 최근 35일의 급여관리자 수기 생성 감사 중
생성 로컬 날짜와 다른 매월 1일이 같은 테넌트·같은 날짜의 서로 다른 직원 2명
이상에 남아 있는 경우에만 검토 경고를 낸다. 이는 날짜 오류 확정이나 자동 정정이
아니다. 정상적인 단일 과거 입력은 경고하지 않으며, 삭제되었거나 다른 날짜로
정정된 행도 현재 상태 재확인에서 제외한다. Slack 수락 뒤에는 tenant/date와 현재
record ID 집합의 PII 없는 fingerprint를 `alerts.work_record_date_slack`에 기록해 같은
집합을 반복 발송하지 않는다. 새 의심 행이 추가되어 집합이 바뀌면 다시 경고한다.
