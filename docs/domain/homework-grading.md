...
무효화 표시만 남아 있어도 변경값이 없으면 안전하게 재사용한다. 변경을 담은 만료
초안은 새 기기의 조회에 복구 대상으로 노출하고, 사용자가 복원을 명시한 경우에만
새 client lease로 인계한다. 이전 기기 행은 `SAME_ACCOUNT_HANDOFF`로 무효화하되
payload를 보존한다. 다른 계정·tenant는 조회하거나 인계할 수 없다.
