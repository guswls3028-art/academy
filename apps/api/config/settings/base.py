...
TEMPLATES = [
    {
        ...
        "DIRS": [
            os.path.join(BASE_DIR, "domains", "assets", "omr", "renderer", "templates"),
        ],
        "APP_DIRS": True,
        "OPTIONS": {
            "context_processors": [
...
# 비어 있으면 일반 운영 룰은 전송을 생략한다. 실제 user_incidents가 있으면
# DEV_ALERTS_WEBHOOK_REQUIRED와 무관하게 check_dev_alerts가 실패한다.
...
# Job timeout (참고용; 실제 timeout은 Batch JobDef.timeout=6h에 반영). long path 폐기로 LONG 상수 제거.
